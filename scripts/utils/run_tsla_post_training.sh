#!/bin/bash
# TSLA post-training pipeline — sampling + validation + cross-ticker eval,
...
#   ./scripts/utils/run_tsla_post_training.sh \
#       2>&1 | tee logs/tsla_post_$(date +%Y%m%d_%H%M%S).log
#   # Ctrl-b d
#
# Stages:
#   1. Wait for tsla_v2 + tsla_v5 trainings to finish (or detect they have)
...
# ══════════════════════════════════════════════════════════════════════════
# STAGE 1 — Wait for TSLA trainings (or skip if already done)
# ══════════════════════════════════════════════════════════════════════════
log "════════════════════════════════════════════════════════════════"
log "STAGE 1 — wait for tsla_v2 + tsla_v5 trainings"
log "════════════════════════════════════════════════════════════════"

# Find any RUNNING/PENDING ph2_finetune jobs on tsla_* checkpoints
...
for model in v2 v5; do
    c=$(best_ckpt "$CKPT_ROOT/tsla_$model")
    if [[ -z "$c" ]]; then
        log "  ✗ tsla_$model: NO CHECKPOINT under $CKPT_ROOT/tsla_$model"
        continue
    fi
    CKPT[$model]="$c"
    log "  ✓ tsla_$model: $c"
...
for model in "${!CKPT[@]}"; do
    synth_root="$SYNTH_ROOT/tsla_$model"
    synth_dir="$synth_root/$TICKER"
    n_existing=$(find "$synth_dir" -name '*.parquet' 2>/dev/null | wc -l | tr -d ' ')
    if [[ $n_existing -ge 1000 ]]; then
        log "  [tsla_$model] $n_existing parquets already exist — skipping sample"
        continue
    fi
    log "  [tsla_$model] submitting 400-task sampling array"
    jid=$(sbatch --parsable \
        --partition=mit_preemptable \
        --array=0-399 \
        --job-name=tsla_${model}_sample \
        --output=logs/tsla_${model}_sample_%A_%a.out \
        --error=logs/tsla_${model}_sample_%A_%a.err \
...
for model in "${!CKPT[@]}"; do
    synth_dir="$SYNTH_ROOT/tsla_$model/$TICKER"
    n=$(find "$synth_dir" -name '*.parquet' 2>/dev/null | wc -l | tr -d ' ')
    log "  [tsla_$model] $n parquets in $synth_dir"
    if [[ $n -lt 100 ]]; then
...
for model in "${!CKPT[@]}"; do
    synth_dir="$SYNTH_ROOT/tsla_$model/$TICKER"
    out_dir="results/_archive/tsla_$model/validation"
    n=$(find "$synth_dir" -name '*.parquet' 2>/dev/null | wc -l | tr -d ' ')
    if [[ $n -lt 100 ]]; then
        log "  [tsla_$model] insufficient synth — skipping validation"
        continue
    fi
    if [[ -f "$out_dir/G1_pass_fail_summary.csv" ]]; then
        log "  [tsla_$model] validation output exists — skipping"
        continue
    fi
    mkdir -p "$out_dir"
    cmd="SYNTH_DIR=$synth_dir OUT_DIR=$out_dir TICKER=$TICKER N_SYNTH=250 \
         sbatch --partition=mit_preemptable --mem=64G --time=01:00:00 \
                --cpus-per-task=4 \
                --job-name=tsla_${model}_validate \
                --output=logs/tsla_${model}_validate_%j.out \
                --error=logs/tsla_${model}_validate_%j.err \
    ...
    if [[ -n "$jid" ]]; then
        ...
        VAL_NAMES[$jid]="tsla_${model}_validate"
    fi
done
...
DIRS=()
for model in "${!CKPT[@]}"; do
    d="$SYNTH_ROOT/tsla_$model/$TICKER"
...
POST_JIDS=""
if [[ ${#LABELS[@]} -gt 0 ]]; then
    out_trades="results/trades_metrics_tsla"
    out_pred="results/predictive_score_tsla"
    ...
    jid=$(submit_one tsla_trades_metrics 128G 2 "" "$cmd_t") && POST_JIDS="$POST_JIDS $jid"
    ...
    jid=$(submit_one tsla_predictive_score 64G 2 "" "$cmd_p") && POST_JIDS="$POST_JIDS $jid"
fi
...
log "════════════════════════════════════════════════════════════════"
for model in v2 v5; do
    val_csv="results/_archive/tsla_$model/validation/G1_pass_fail_summary.csv"
    n_synth=$(find "$SYNTH_ROOT/tsla_$model/$TICKER" -name '*.parquet' 2>/dev/null | wc -l | tr -d ' ')
    [[ -f "$val_csv" ]] && val_status="✓ ${val_csv}" || val_status="✗ MISSING"
    log "  tsla_$model:  $n_synth synth parquets   |  validation: $val_status"
done
log ""
log "Cross-ticker outputs:"
for f in results/trades_metrics_tsla/{trades_metrics_summary.txt,pca_coverage.csv} \
         results/predictive_score_tsla/predictive_score.csv; do
    [[ -f "$f" ]] && log "  ✓ $f" || log "  ✗ $f MISSING"
done
log ""
log "Pull to Mac:"
log "  rsync -avz favara@orcd-login.mit.edu:diff_mm/results/{_archive/tsla_*,trades_metrics_tsla,predictive_score_tsla}/ \\"
